...
class AttentionGate(nn.Module):
    def __init__(self, skip_channels, gate_channels, intermediate_channels=16):
        ...
        self.upsample_gating_signal_to_skip_resolution = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)

    def forward(self, skip_features, gating_signal):
        upsampled_gating_projection = self.upsample_gating_signal_to_skip_resolution(
            self.project_gating_signal_to_intermediate(gating_signal)
        )
        skip_projection = self.project_skip_features_to_intermediate(skip_features)

        combined_projections = torch.relu(skip_projection + upsampled_gating_projection)
...
class MicroUNet(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        return self.final_one_by_one_segmentation_head(current_feature_map)
